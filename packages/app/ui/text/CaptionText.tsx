...
export const CaptionText = ({ children, size, className, weight, as }: CaptionTextProps) => {
  const TextComponent = as || "p";
  return (
    <TextComponent className={cx(boldStyles({ weight }), captionTextStyles({ size, className }))}>
      {children}
    </TextComponent>
  );
};
