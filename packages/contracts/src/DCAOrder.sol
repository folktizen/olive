...
contract DCAOrder is IConditionalOrder, EIP1271Verifier, IDCAOrder {
    ...
    function initialize(
        ...
        IERC20(_sellToken).safeApprove(
            address(IGPv2Settlement(_settlementContract).vaultRelayer()),
            type(uint256).max
        );
        ...
        returns (GPv2Order.Data memory)
    {
        ...
        // solhint-disable-next-line not-rely-on-time
        uint32 currentTimeBucket = ((uint32(orderExecutionTime) / 900) + 1) *
            900;
        return
            GPv2Order.Data(
                sellToken,
    ...
    function isValidSignature(
        ...
        require(
            IConditionalOrder(this).getTradeableOrder().hash(domainSeparator) ==
                orderDigest,
