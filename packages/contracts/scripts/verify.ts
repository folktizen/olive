...
const CHAIN_INFO: Record<string, { chainId: number; oklinkUrl: string }> = {
  ethereum: { chainId: 1, oklinkUrl: 'https://www.oklink.com/api/v5/explorer/contract/verify-source-code-plugin/eth' },
...
function usage(): never {
  console.log(`Usage: bun run scripts/verify.ts <verifier> <api-key>\n  <verifier>: etherscan | oklink\n  <api-key>: Your API key for the selected verifier\n  `);
  process.exit(1);
}
...
const contracts: Array<{ chain: string; dcaOrder: string; orderFactory: string }> = JSON.parse(fs.readFileSync(CONTRACTS_PATH, 'utf8'));
...
for (const entry of contracts) {
  ...
  if (!info) {
    ...
    console.warn(`Unknown chain: ${chain}, skipping...`);
    continue;
  }

  // For each contract type, build and run the verification command.
  for (const [name, address] of Object.entries({ DCAOrder: dcaOrder, OrderFactory: orderFactory })) {
